...
// Source: src/app/(frontend)/en/proizvodi/[category]/page.tsx
// Variable: CATALOG_CATEGORY_QUERY
// Query: *[	_type == 'product.category' && showInCatalog == true && slug_en.current == $slug][0]{	_id, 'title': title_en, 'description': description_en, 'slug': slug_en.current,	image{..., asset->},	'products': *[		_type == 'product' && hidden != true && language == 'en'		&& category._ref == ^._id && defined(metadata.slug.current)	]|order(title asc){		_id, title, image{..., asset->},		'slug': '/products/en/' + metadata.slug.current	}}
export type CATALOG_CATEGORY_QUERY_RESULT = {
	_id: string
	title: string | null
	description: string | null
	slug: string | null
...
// Source: src/ui/modules/product/category-list.tsx
// Variable: CATALOG_CATEGORIES_QUERY
// Query: *[	_type == 'product.category' && showInCatalog == true && defined(slug_en.current)]|order(coalesce(sortOrder, 0) asc, title_en asc){	_id, 'title': title_en, 'description': description_en, 'slug': slug_en.current,	image{..., asset->}}
...
// Query: *[		_type == 'product' && hidden != true		&& _id != $id		&& category._ref == $categoryId		&& coalesce(language, $defaultLang) == $lang	]|order(title asc)[0...$limit]{		_id,		title,		image{			...,			asset->		},		'slug': select(			$lang == 'hr' => $productsDir + metadata.slug.current,			$productsDir + $lang + '/' + metadata.slug.current		),	}
...
// Query: *[		_type == 'product.category'		&& count(*[_type == 'product' && hidden != true && references(^._id) && coalesce(language, $defaultLang) == $lang]) > 0	]{		_id,		'title': select(			$lang == 'en' => coalesce(title_en, title),			title		),		'slug': select(			$lang == 'en' => coalesce(slug_en.current, slug.current),			slug.current		),	}|order(title asc)
